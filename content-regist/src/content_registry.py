...
@app.callback(
    ...
    Input("table-contents-memo", "data"),
    Input("tab-group", "value"),
    Input("monitoring", "n_intervals"),
)
def refresh_table(data, tab_value, n_intervals):
    model_list = get_dash_table_data(
        collection=tab_value + "s",
        fields={key: 1 for key in ContentVariables.MODEL_KEYS},
    )
    return model_list
...
@app.callback(Output("collapse-app-port", "is_open"), Input("app-type", "value"))
...
def update_layout(tab_value):
    if tab_value == "model":
        return (
            ...
            False,
            False,
            [{"id": p, "name": p} for p in ContentVariables.MODEL_KEYS],
        )
    elif tab_value == "app":
        return (
            dash_forms("app"),
            ...
            True,
            False,
            [{"id": p, "name": p} for p in ContentVariables.APP_KEYS],
        )
        ...
        return (
            ...
            False,
            True,
            [{"id": p, "name": p} for p in ContentVariables.WORKFLOW_KEYS],
        )
...
def display_dynamic_component(n_clicks, children):
...
def display_output(value, n_cliks):
    ...
    if value in ["int", "float", "str"]:
        return dbc.Card(
            [
                dbc.Label(
                    "GUI paramerers for {} component".format(value), className="mr-2"
                ),
                make_form_input(i),
            ]
        )
...
def display_workflow_component(n_clicks, children):
    new_element = html.Div(
        [
            ...
            html.Div(
                [
                    dcc.Dropdown(
                        id={"type": "content-type-dropdown", "index": n_clicks},
                        options=[
                            {"label": i, "value": i}
                            for i in ["models", "apps", "workflows"]
                        ],
                        style={"width": "80%"},
                        value="workflows",
                    ),
                    dcc.Dropdown(
                        id={"type": "content-dropdown", "index": n_clicks},
            ...
            html.Div(id={"type": "workflow-dynamic-output", "index": n_clicks}),
        ]
    )
    children.append(new_element)
    return children
...
def json_generator(
    ...
    if "generate-json" in changed_id or "gui-check" in changed_id:
        ...
        if "compute_resources" in json_document:
            if content_type == "model":
                json_document["compute_resources"] = {
                    "num_processors": model_cpu_num,
                    "num_gpus": model_gpu_num,
                }
            else:
                json_document["compute_resources"] = {
                    "num_processors": app_cpu_num,
        ...
        if content_type == "model":
            if (
                children[0]["props"]["children"][2]["props"]["children"] is not None
                and component_type
            ):
                for k, child in enumerate(children):
                    input_items = child["props"]["children"][2]["props"]["children"][
                        "props"
                    ...
                    for input_item in input_items:
                        if "value" in input_item["props"]:
                            ...
                            if input_type == "marks":
                                ...
                                for leng in range(int(length / 2)):
                                    marks[input_value[2 * leng]] = input_value[
                                        2 * leng + 1
                                    ]
                                ...
                                for leng in range(int(length / 2)):
                                    options.append(
                                        {
                                            "label": input_value[2 * leng],
                                            "value": input_value[2 * leng + 1],
                                        }
                                    )
                    ...
                    if component_combo["type"] == "int" and "value" in component_combo:
                        ...
                        component_combo["type"] == "float"
                        and "value" in component_combo
                    ):
                        component_combo["value"] = float(component_combo["value"])
                    elif (
                        component_combo["type"] == "bool" and "value" in component_combo
                    ):
            ...
            if workflow_children is not None:
                for child in workflow_children:
                    workflow_list.append(
                        child["props"]["children"][1]["props"]["children"][1]["props"][
                            "value"
                        ]
                    )
    ...
    if "button-upload.n_clicks" in changed_id:
        if json_document and is_valid:
            ...
            send_webhook(
                {
                    "event": "add_content",
                    "content_id": json_document["content_id"],
                    "content_type": json_document["content_type"],
                }
            )
...
def show_gui_layouts_check(n_clicks, data):
    changed_id = [p["prop_id"] for p in ctx.triggered][0]
    if "gui-check" in changed_id and "gui_parameters" in data:
        if data["gui_parameters"]:
            item_list = JSONParameterEditor(
                ...
                json_blob=remove_key_from_dict_list(
...
def show_gui_layouts_validation(n_clicks, data):
    changed_id = [p["prop_id"] for p in ctx.triggered][0]
        ...
        if data["gui_parameters"]:
            ...
            if is_valid:
                item_list = JSONParameterEditor(
                    ...
                    },  # pattern match _id (base id), name
                    json_blob=remove_key_from_dict_list(
                        data["gui_parameters"], "comp_group"
                    ),
                )
