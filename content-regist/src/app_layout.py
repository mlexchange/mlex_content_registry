...
def dash_forms(type):
    forms = [
        dbc.Col(
            [
                ...
                dbc.Input(
                    id="name-regist",
                    type="text",
                    placeholder="Enter {} name.".format(type),
                    debounce=True,
                ),
            ],
        ),
        dbc.Col(
            [
                ...
                dbc.Input(
                    id="version-regist",
                    type="text",
                    placeholder="Enter {} version.".format(type),
                    debounce=True,
                ),
            ],
        ),
        ...
        dbc.Col(
            [
                dbc.Label(
                    "Enter the commands to deploy the {}. Use comma to separate.".format(
                        type
                    ),
                    className="mr-2",
                ),
                dbc.Input(
                    id="cmd-regist",
                    type="text",
                    placeholder="Enter commands to deploy the {}".format(type),
                    debounce=True,
                ),
            ],
        ),
        dbc.Col(
            [
                dbc.Label(
                    "Enter any other kwargs to deploy the {} (optional). \
                     Use double quotes to construct the kwargs dictionary.".format(
                        type
                    ),
                    className="mr-2",
                ),
                dbc.Input(
                    id="kwargs-regist",
                    type="text",
                    placeholder="Enter kwargs to deploy the {}".format(type),
                    debounce=True,
                ),
            ],
        ),
        ...
        dbc.Col(
            [
                dbc.Label(
                    "Enter application(s) for the {} (e.g., image segmentation). Use comma to separate.".format(
                        type
                    )
                ),
                dbc.Input(
                    id="application-regist",
                    type="text",
                    placeholder="Enter applications for this {}".format(type),
                    debounce=True,
                ),
            ],
        ),
            ...
            [
                ...
                html.Div(
                    [
                        ...
                        dbc.Input(
                            id="model-cpu-num", type="number", value=0, className="mr-5"
                        ),
    ...
    [
        ...
        dbc.Collapse(
            children=dbc.Col(
                [
                    dbc.Label(
                        "Please input porting for the frontend app. Use comma to separate if there are more than one."
                    ),
                    dbc.Input(
                        id="app-port",
                        type="text",
                        placeholder="Enter port_number/method (e.g. 8061/tcp).",
                        debounce=True,
                    ),
                ],
            ),
    ...
    children=[
        dbc.CardBody(
            [
                ...
                html.Div(
                    id="tab-display", children=dbc.Form(children=dash_forms("model"))
                ),
                dbc.Collapse(
                    MODEL_REGISTRY,
                ...
                dbc.Form(
                    [
                        html.Div(
                            [
                                dbc.Label(
                                    "Please upload your content document after validation."
                                ),
                                dbc.Button(
            ...
            [
                ...
                dbc.Modal(
                    [
                        ...
                        dbc.ModalBody(
                            "Files cannot be recovered after deletion. Do you still want to proceed?"
                        ),
                        dbc.ModalFooter(
                            [
                                dbc.Button(
                ...
                html.Div(
                    children=[
                        dash_table.DataTable(
                            ...
                            columns=[
                                {"id": p, "name": p}
                                for p in ContentVariables.MODEL_KEYS
                            ],
...
meta = [
    html.Div(
        ...
        children=[
            ...
            dcc.Interval(
                id="monitoring", interval=1 * 1000, n_intervals=0  # in milliseconds
            ),
        ],
    ),
]
...
app.layout = html.Div(
    [
        ...
        dbc.Container(
            [
                dbc.Row(
                    [dbc.Col(register_model, width=6), dbc.Col(upload_model, width=6)]
                ),
